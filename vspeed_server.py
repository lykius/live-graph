...
def data_server():
    while True:
        ...
        if label not in data:
            data[label] = deque([0 for _ in range(MAX_DEQUE_SIZE)], MAX_DEQUE_SIZE)
        data[label].append(value)
        client_connection.send('#ACK#'.encode())
        yield data

lines = {}
fig = plt.figure(num='Vertical Speed')
plt.xlim(XRANGE)
plt.ylim(YRANGE)
...
try:
    with server_socket:
        ...
        with client_connection:
            anim = animation.FuncAnimation(fig, animate, data_server, interval=REFRESH_PERIOD, blit=True)
            plt.show()
except:
